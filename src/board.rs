...
impl Plugin for BoardPlugin {
    fn build(&self, app: &mut App) {
        app.init_resource::<SquareMaterials>()
            ...
            .add_system(despawn_taken_pieces.after(move_piece));
    }
}
    ...
    for event in events.iter() {
        match event {
            PickingEvent::Selection(e) => {
                ...
                if let SelectionEvent::JustSelected(selected_entity) = e {
                    let current_square =
                        if let Ok(current_square) = squares_query.get(*selected_entity) {
                            current_square
                        } else {
                            // Nothing selected
                            // not working
                    ...
                    info!("Selecting entity: {}", selected_entity.index());
                    selected_square.entity = Some(*selected_entity);
            ...
            PickingEvent::Hover(e) => {
                ...
                if selected_square.entity == Some(entity) {
                    ...
                    if *hovered_entity == entity {
                        let material = materials.get_mut(material_handle).unwrap();
                        info!("Just Entered Entity: {}", entity.index());
                    ...
                    if *hovered_entity == entity {
                        ...
                        material.base_color = if square.is_white() {
                            info!("Just Left entity: {} which was white", entity.index());
                            Color::rgb(1., 0.9, 0.9)
                        } else {
                            info!("Just Left entity: {} which was black", entity.index());
                            Color::rgb(0., 0.1, 0.1)
                        };
                    }
                }
